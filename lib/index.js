...
function bundleFile(entry, outputPath, noMinify) {
  return new Promise((resolve, reject) => {
    const output = bundle(createGraph(entry), noMinify)
    fse
      .outputFile(outputPath, output.code)
      .then(() => {
        resolve({ code: output.code, dependecies: output.dependecies })
      })
      .catch(err => {
        reject(err)
      })
